...
This is the async port used by the gate cycle in API-Trafix. It is byte-for-byte
the same logic as ``trafix-api-mock/trafix/rates.py``; only the tariff source
changes (``parking_rates`` instead of ``parking_fees``, so the progressive
fields are absent and default to 0).
...
# Storage values on parking_rates.fee_category -> the wire/display names above.
_CATEGORY_TO_WIRE = {"flat": FEE_FLAT, "progresif": FEE_PROGRESSIVE}


@dataclass(frozen=True)
class Tariff:
    """The subset of ``parking_rates`` the calculation actually reads."""
    ...
    def from_row(cls, row) -> Tariff:
        """Build a tariff from a :class:`api_trafix.models.ParkingRate`.

        The progressive fields do not exist on ``parking_rates`` yet, so they
        default to 0 — a Flat tariff never reads them, and no progressive
        tariff can be created until those columns are added.
        """
        category = row.fee_category or FEE_FLAT
        return cls(
            fee_category=_CATEGORY_TO_WIRE.get(category, category),
            grace_periode=int(row.grace_period_minutes or 0),
            fee_first_time=0,
            fee_first_price=float(row.base_price or 0),
            fee_time_1=0,
            fee_price_1=0,
            fee_price_max=0,
...
def is_active_member(
    ...
    if member is None:
        return False
    today = today or date.today()  # noqa: DTZ011 - injectable clock for tests
