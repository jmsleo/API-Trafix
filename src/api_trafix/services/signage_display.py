...
Replaces the Vala signage app's MQTT-based state management with a
Redis pub/sub system that pushes updates to web displays via SSE.
...
class SignageDisplayService:
    ...
    async def update_status(self, gate_code: str, status: str, **kwargs: Any) -> None:
        ...
        await self._publish(SIGNAGE_TEXT_CHANNEL, {
            "gate": gate_code,
            "status": status,
        ...
        await self._publish_mqtt_status(gate_code, status)
    ...
    async def update_ads(self, gate_code: str, ads: list[dict[str, Any]]) -> None:
        ...
        await self._publish(SIGNAGE_ADS_CHANNEL, {
            "gate": gate_code,
            "ads": ads,
            "timestamp": state.last_updated.isoformat(),
        })
    ...
    async def update_idle(self, gate_code: str, image: dict[str, Any] | None) -> None:
        ...
        await self._publish(SIGNAGE_IDLE_CHANNEL, {
            "gate": gate_code,
            "image": image,
            "timestamp": state.last_updated.isoformat(),
        })
    ...
    async def update_media(self, gate_code: str, media: list[dict[str, Any]]) -> None:
        ...
        await self._publish(SIGNAGE_MEDIA_CHANNEL, {
            "gate": gate_code,
            "media": media,
            "timestamp": state.last_updated.isoformat(),
        })
    ...
    async def _publish_mqtt_status(self, gate_code: str, status: str) -> None:
        """Publish status to MQTT for backward compatibility."""
        # This will be called by the orchestrator's signage publisher
        # For now, just log it
        logger.debug("MQTT status would be published: gate=%s status=%s", gate_code, status)

    def subscribe(self, gate_code: str) -> asyncio.Queue:
        """Subscribe to updates for a specific gate."""
        queue: asyncio.Queue = asyncio.Queue()
        self._subscribers[gate_code] = queue
        return queue

    def unsubscribe(self, gate_code: str) -> None:
